...
def load_live_data():
    try:
        ...
        if 'actions_done' not in df.columns and len(df.columns) > 1:
            df['actions_done'] = df.iloc[:, 1] # ستون دوم را به عنوان لاگ بگیر
...
if not df.empty:
    ...
    # 🌟 ۳. دسته‌بندی پیشرفته با منطق فازی (بهبود یافته برای مهار غلط‌های املایی تکنسین‌ها)
    def fuzzy_categorize_text_log(text):
        # نمونه اصلاح‌شده شرط دسته‌بندی در کدت
        text_lower = str(text).lower()
        words = text_lower.split()
        THRESHOLD = 85.0 # درصد شباهت برای مهار غلط‌های املایی
            ...
            with col_chart2:
                ...
                if 'actual_label' in matched_df.columns:
                    ...
                    if len(clean_matched) > 0:
                        ...
                        # ۲. رسم خودِ ماتریس درهم‌ریختگی روی صفحه (کدی که پاک شده بود)
    ...
    if not df.empty:
        def calculate_metrics(data):
            if 'actual_label' in data.columns:
                correct = (data['standardized_action'] == data['actual_label']).sum()
        ...
        #else:
            #st.sidebar.info("Upload 'actual_label' column for accuracy tracking.")
...
>>>>>>> 54d7652 (Update README and add Offline_Portal analytical module)
